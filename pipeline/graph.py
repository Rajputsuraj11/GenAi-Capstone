...
def call_with_retry(fn, state_copy, retries=3):
...
def parallel_scan_node(state: ComplianceState) -> ComplianceState:
    ...
    Pages also run in parallel with max_workers=3 to respect Groq free tier TPM limit.
    """
    all_pii_findings = []
    ...
    def scan_single_page(page):
        """Scan a single page with all 3 LLM agents in parallel."""
        ...
        # Run 3 LLM agents in parallel for this page
        with ThreadPoolExecutor(max_workers=3) as agent_executor:
            futures = {}

            # Submit all 3 LLM agents
            futures[agent_executor.submit(call_with_retry, pii_check_node, copy.deepcopy(single_page_state))] = "pii"
            futures[agent_executor.submit(call_with_retry, confidential_check_node, copy.deepcopy(single_page_state))] = "confidential"
            futures[agent_executor.submit(call_with_retry, abusive_check_node, copy.deepcopy(single_page_state))] = "abusive"
    ...
    # This respects Groq free tier TPM limit of 6000/min
    page_results = []
...
def build_smart_compliance_graph():
    """
    Parallel compliance graph:
    - Runs PII, Confidential, Abusive agents in parallel per page
    - Pages also run in parallel with max_workers=3 (respects Groq TPM limit)
